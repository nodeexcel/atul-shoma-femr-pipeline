"""
FEMR NetSuite Report Generator (v10)
=====================================
Matches the new FEMR Export Template 041526.xlsx layout.

Changes from v9 (chart aesthetic — matched exactly from FEMR Export Template 041526.xlsx):
  - No chart title (was "Cumulative Project Financials")
  - No axis titles on X or Y (was "Quarter" / "Amount ($)")
  - Removed y-axis numFmt override — let Excel auto-format tick values
  - Legend back to bottom ("b") — no overlap since axis titles removed
  - Chart width=15, height=7.5 (was 22×10)
  - Series colors: theme accent1–5 per series (matches Josh's template exactly)
  - Marker size: 5 (was 6), line width: 9525 EMU (matches template)
...
def _add_line_chart(ws, fiscal_years: list, latest_quarter: str = "Q4"):
    ...
    # No chart title, no axis titles — matches template exactly
...
def build_tab(wb: Workbook, meta: dict, data: dict,
    ...
    # Cumulative section
    _write_cumulative_section(ws, fiscal_years, col_map, latest_quarter)
...
def process_sequence(wb: Workbook, seq_entry: dict, fiscal_years: list,
    ...
    if not multi:
        ...
        build_tab(wb, meta, data, fiscal_years, name, latest_quarter)
        return 1
...
def run(mapping_path: str, output_prefix: str,
    ...
    logger.info("=== FEMR NetSuite Report Generator (v10) ===")
