"""
FEMR NetSuite Report Generator (v5)
====================================
...
Usage:
    python femr_netsuite_report_5.py --sequence 2ADP001        # single tab test
    python femr_netsuite_report_5.py --group ADP               # all ADP sequences
    python femr_netsuite_report_5.py                           # all 5 groups
    python femr_netsuite_report_5.py --workers 30              # tune concurrency
"""
...
def fetch_sequence_metadata(sequence: str) -> list:
    """
    Query MV filtered by sequence with limit=1. Returns list with one metadata
    dict (the primary rollup for the sequence).

    Note: MV pagination is very slow (~26s/page). For most sequences there's
    only one rollup, so a single-row fetch is sufficient. Multi-rollup
    sequences would need a different strategy (e.g. preloaded rollup list).
    """
...
def process_sequence(wb: Workbook, seq_entry: dict, fiscal_years: list,
    ...
    rollups_meta = fetch_sequence_metadata(sequence)
    if not rollups_meta:
...
def run(mapping_path: str, output_prefix: str,
    ...
    logger.info("=== FEMR NetSuite Report Generator (v5) ===")
    ...
    for group, seqs in sorted(by_group.items()):
        ...
        for i, s in enumerate(seqs, 1):
            ...
            n = process_sequence(wb, s, fiscal_years, workers)
            total_tabs += n
            if i % 10 == 0:
                wb.save(fname)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", "-o", default="femr_v5_report",
