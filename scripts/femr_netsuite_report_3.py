...
Row layout per tab (matches ADP 116 / ADP 33 template):
  Row 1     : NextFlex
  Row 2     : ADP  | number | Rollup (if applicable)
  Row 3     : Project Name | value
  Row 4     : Type B | value
  Row 5     : Type C | value
  Row 6     : Color of Money | value
  Row 7     : ACRNs | value
  Row 8     : (blank)
  ...
  Row 11-24 : Actuals section (A11:A24 merged = "Actuals")
              B = account name, C = account code, D onwards = data
  ...
  Row 49    : formulas | Total Committed | values...
...
Usage:
    ...
    python femr_netsuite_report.py                           # all sequences
...
def _write_data_row(ws, row: int, data: dict, fye_q_pairs: list,
    ...
    """Write one data row across all FYE/Quarter columns."""
...
def _write_formulas_section(ws, fiscal_years: list, col_map: dict,
                             ...
                             expended_row: int):
    """
    Write the formulas summary section starting at row 48.
    Columns: B=label, then one col per FYE/Quarter (same layout as main data)
    """
    header_row = 48
    ...
    # Formula rows — reference the data rows above
    formula_rows = [
    ...
    for (frow, label, src_row) in formula_rows:
        ...
        for year in fiscal_years:
            for q in QUARTERS:
                ...
                c = ws.cell(row=frow, column=col,
                            value=f"={col_letter}{src_row}")
                c.number_format = DASH_FMT
                c.alignment = RIGHT
                col += 1
...
def build_adp_sheet(wb: Workbook, sequence: str, meta: dict,
    ...
    # ── Rows 1–7: Header block ────────────────────────────────────────────
    header_fields = [
        ("NextFlex",        ""),
        ("ADP",             meta["adp"]),
        ("Project Name",    meta["name"]),
        ("Type B",          meta.get("type_b") or ""),
        ("Type C",          meta.get("type_c") or ""),
        ("Color of Money",  ""),
    ...
    fye_q_pairs = [(f"FYE 9/30/{y}", q) for y in fiscal_years for q in QUARTERS]
    ...
    # ── Rows 48–54: Formulas summary section ─────────────────────────────
...
def _chunk_sequences_by_adp_range(sequences: list, chunk_size: int = 50) -> list:
    ...
    """
    # Separate numeric ADP sequences from non-numeric (PC etc.)
    ...
    # Non-numeric sequences (PC tabs) go into their own file if any
    if non_numeric:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate FEMR NetSuite multi-tab Excel report, split into files of 50 ADPs."
    )
