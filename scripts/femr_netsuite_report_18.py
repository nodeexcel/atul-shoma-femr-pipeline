"""
FEMR NetSuite Report Generator (v18)
=====================================
Matches the new FEMR Export Template 041526.xlsx layout.

Changes from v17:
  - Account row labels reformatted: GL code first, then description, sorted
    numerically (e.g. "5001 Labor Cost" instead of "Labor Cost 5001").
    Confirmed against Josh's sample file FEMR Dev_QA v051326.xlsx (2026-05-07).
  - Labor Hours statistical account row removed entirely. Actuals and Budgeted
    sections now have 12 data rows each instead of 13. All row constants below
    the actuals section shift up by 1.
  - Dollar sign formatting split into two formats: individual Actuals/Budgeted
    data cells use NUM_FMT_NO_DOLLAR (no $ symbol); total rows (ACTUALS Total,
    BUDGETED Total) and Grand Total column keep NUM_FMT (with $). Contracting
...
Changes from v16:
  - Fixed missing OAuth header in _fetch_mv_by_identifier(). The MV endpoint
    also requires Bearer token auth, but v16 only added auth to _http_get() and
    _fetch_netamount(). Multi-identifier sequences (e.g. 2ADP099) use the MV
    path and were getting HTTP 401 on every call, resulting in empty tabs.
    Fix: pass _get_auth_header() to the urllib.request.Request in that function.

Changes from v15:
  - Oracle APEX API endpoints are now OAuth-secured (implemented 2026-04-29).
    Script reads ORACLE_CLIENT_ID and ORACLE_SECRET_KEY from the environment,
    fetches a Bearer token via client_credentials grant at startup, and attaches
    it to every API request. Token is auto-refreshed 5 minutes before expiry so
    long-running ADP runs (8-9 hours) never hit a 401 mid-flight.
...
# ── OAuth token state (module-level, shared across all threads) ───────────────
_token_state = {"token": None, "expires_at": 0.0}
_token_lock  = __import__("threading").Lock()


def _fetch_oauth_token() -> tuple:
    """POST client_credentials to Oracle APEX token endpoint. Returns (token, expires_in)."""
    client_id  = os.environ.get("ORACLE_CLIENT_ID", "").strip()
    client_secret = os.environ.get("ORACLE_SECRET_KEY", "").strip()
    if not client_id or not client_secret:
        raise RuntimeError(
            "ORACLE_CLIENT_ID and ORACLE_SECRET_KEY must be set in the environment. "
            "Copy .env.example to .env and fill in the credentials from Josh."
        )
...
def _get_auth_header() -> dict:
    """Return Authorization header, refreshing the token if within 5 minutes of expiry."""
    with _token_lock:
        if time.time() >= _token_state["expires_at"] - 300:
            token, expires_in = _fetch_oauth_token()
            _token_state["token"] = token
            _token_state["expires_at"] = time.time() + expires_in
            logger.info("OAuth token (re)fetched — valid for %ds", expires_in)
    return {"Authorization": f"Bearer {_token_state['token']}"}
...
NUM_FMT_NO_DOLLAR = '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'
...
def _write_data_row(ws, row: int, data: dict, fye_q_pairs: list,
    ...
    for (fye, q) in fye_q_pairs:
        ...
        if amount != 0.0:
            ...
            c.number_format = NUM_FMT_NO_DOLLAR
            c.alignment = RIGHT

    # Grand Total — individual data rows use no-dollar format
    ...
    if total_col and first_col and last_col:
        ...
        tc.number_format = NUM_FMT_NO_DOLLAR
        tc.alignment = RIGHT
        tc.fill = GREY_FILL


def _write_section_total_row(ws, row: int, data_start_row: int, col_map: dict):
    """Write a SUM total row across all account rows."""
...
def run(mapping_path: str, output_prefix: str,
    ...
    logger.info("=== FEMR NetSuite Report Generator (v18) ===")
