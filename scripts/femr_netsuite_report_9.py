"""
FEMR NetSuite Report Generator (v9)
====================================
Matches the new FEMR Export Template 041526.xlsx layout.

Changes from v8 (per Josh's feedback / meeting notes Q6):
  - Dynamic quarter range: detect latest quarter with data from MV at startup
    instead of hardcoding to Q4 FY2026. Columns now stop at the last quarter
    that has any data (currently Q2 FY2026 per Josh). Fixes cumulative rows
    carrying forward into future empty quarters.
...
def _add_line_chart(ws, fiscal_years: list, latest_quarter: str = "Q4"):
    ...
    # Legend at right — avoids overlap with x-axis quarter labels at bottom
    if chart.legend is not None:
        chart.legend.position = "r"

    # Number of quarter columns = full years * 4 + partial last year
    last_q_idx = QUARTERS.index(latest_quarter)
...
def run(mapping_path: str, output_prefix: str,
    ...
    logger.info("=== FEMR NetSuite Report Generator (v9) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", "-o", default="femr_v8_report",
